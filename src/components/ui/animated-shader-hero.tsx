...
const AnimatedShaderHero: React.FC<HeroProps> = ({
  ...
  return (
    <div className={`relative w-full min-h-screen overflow-hidden bg-background ${className}`}>
      <canvas
        ref={canvasRef}
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pt-20 md:pt-32 pb-12 md:pb-20 px-4 md:px-6">
        ...
        <div className="text-center space-y-6 md:space-y-8 max-w-5xl mx-auto px-2">
          {/* Main Heading with Animation */}
          <h1 className="text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-black leading-tight animate-fade-in-up animation-delay-200">
            <span className="text-gradient block mb-2">{headline.line1}</span>
          ...
          {buttons?.primary && (
            <div className="pt-4 md:pt-6 animate-fade-in-up animation-delay-800">
              <button 
                onClick={buttons.primary.onClick}
                className="bg-gradient-to-r from-primary to-secondary hover:opacity-90 text-white px-4 md:px-8 lg:px-12 py-5 md:py-6 text-sm md:text-base lg:text-lg font-black rounded-full glow-effect w-full sm:w-auto transition-all duration-300 hover:scale-105"
              >
...
// Shader adaptado para cores roxas/azuis do Dark Planner
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		vec2 p=uv;
		float d=length(p);
		// Tons azuis vibrantes mantendo a paleta
		col+=.0015/d*(cos(sin(i)*vec3(0.5,1.8,3.2))+1.);
		float b=noise(i+p+bg*1.731);
		col+=.0025*b/length(max(p,vec2(b*p.x*.02,p.y)));
		// Mix com azul escuro vibrante
		col=mix(col,vec3(bg*.06,bg*.14,bg*.38),d);
	}
	O=vec4(col,1);
}`;
